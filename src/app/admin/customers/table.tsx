...
export function CustomersTable({ data }: { data: any[] }) {
  const columns: Column<any>[] = [
    ...
    { key: "type", header: "Tipo", sortable: true, cell: r => <Badge variant="outline">{r.type}</Badge> },
    ...
    { key: "status", header: "Stato", sortable: true, cell: r => <Badge variant={r.status === "ACTIVE" ? "success" : r.status === "BLOCKED" ? "destructive" : "muted"}>{r.status}</Badge> },
