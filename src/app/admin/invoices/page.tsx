...
const FILTERABLE = ["number", "type", "sdiStatus", "paymentStatus"];
...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const ids = await ftsMatchingIds("Invoice", s.tenantId, p.q, [
  ...
  const columns: ColumnDef<typeof rows[number]>[] = [
    { key: "number", label: "Numero", sortable: true, filter: { type: "text", placeholder: "Num." }, className: "font-mono", render: inv => inv.number },
    {
      ...
      filter: { type: "select", placeholder: "Tutti", options: [
        ...
        { value: "TD24_DEFERRED", label: "TD24 differita" },
    ...
    { key: "issueDate", label: "Data", sortable: true, className: "text-xs", render: inv => formatDate(inv.issueDate) },
    { key: "dueDate", label: "Scadenza", sortable: true, className: "text-xs", render: inv => inv.dueDate ? formatDate(inv.dueDate) : "—" },
    { key: "total", label: "Totale", sortable: true, className: "text-right font-semibold", headerClassName: "text-right", render: inv => formatCurrency(inv.total) },
    {
      ...
      filter: { type: "select", placeholder: "Tutti", options: [
        ...
        { value: "NOT_DELIVERED", label: "Non consegnata" },
      ]},
    ...
    <div className="space-y-4">
      ...
      {total === 0 && !p.q && Object.keys(p.filters).length === 0 ? (
        ...
        <DataTable
          ...
          rowKey={inv => inv.id}
          params={p}
          searchPlaceholder="Cerca per numero, riferimento, cliente…"
        />
      )}
    </div>
  );
