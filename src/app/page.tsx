...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-white text-slate-900">
      <header className="border-b border-slate-200/60 backdrop-blur sticky top-0 z-30 bg-white/80">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 font-display font-bold text-xl text-ampera-700">
            <Zap className="h-6 w-6 text-ampera-700 fill-ampera-700/20" />
            Ampera
          </Link>
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            <a href="#features" className="hover:text-ampera-700">Funzionalità</a>
            <a href="#pricing" className="hover:text-ampera-700">Prezzi</a>
            <Link href="/login" className="hover:text-ampera-700">Accedi</Link>
          ...
          <Link href="/login" className="md:hidden bg-ampera-700 text-white px-3 py-1.5 rounded-lg text-sm font-semibold">Accedi</Link>
        </div>
      </header>

      <section className="relative overflow-hidden bg-gradient-to-b from-slate-50 to-white">
        <div className="container mx-auto px-6 py-20 md:py-32 text-center">
          <span className="inline-flex items-center gap-2 text-xs font-semibold bg-ampera-50 text-ampera-700 px-3 py-1 rounded-full mb-6">
            <Sparkles className="h-3 w-3" /> AI-native · DICO in 5 minuti · Cloud o self-hosted
          </span>
          <h1 className="font-display text-5xl md:text-7xl font-bold tracking-tight mb-6 bg-gradient-to-r from-ampera-800 via-ampera-700 to-site bg-clip-text text-transparent">
      ...
      <section id="features" className="py-20 container mx-auto px-6">
        ...
        <p className="text-center text-slate-600 mb-14">13 moduli, mobile-first, pronto in 30 minuti.</p>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(f => (
            <div key={f.title} className="bg-white border border-slate-200 rounded-xl p-6 lift">
              <div className="h-10 w-10 rounded-lg bg-ampera-50 text-ampera-700 flex items-center justify-center mb-4">{f.icon}</div>
      ...
      <section id="pricing" className="py-20 bg-slate-50">
        <div className="container mx-auto px-6">
          ...
          <p className="text-center text-slate-600 mb-12">A differenza dei competitor, qui i prezzi li vedi prima di chiamarci.</p>
          <div className="grid md:grid-cols-4 gap-4 max-w-6xl mx-auto">
            {plans.map(p => (
              <div key={p.name} className={`bg-white rounded-xl p-6 border ${p.featured ? "border-ampera-700 ring-2 ring-ampera-700/20 shadow-xl" : "border-slate-200"} lift`}>
                <div className="font-semibold text-slate-500">{p.name}</div>
                <div className="font-display text-4xl font-bold my-2">{p.price}<span className="text-sm font-normal text-slate-500">{p.unit}</span></div>
                <div className="text-sm text-slate-600 mb-4">{p.tagline}</div>
                <ul className="text-sm space-y-1.5 mb-5 text-slate-700">
                  {p.features.map(f => <li key={f}>· {f}</li>)}
                </ul>
                <Link href="/login" className={`block text-center rounded-lg py-2.5 font-semibold ${p.featured ? "bg-ampera-700 text-white hover:bg-ampera-800" : "bg-slate-100 hover:bg-slate-200 text-slate-900"}`}>
                  {p.cta}
                </Link>
              </div>
            ))}
      ...
      <footer className="border-t border-slate-200 py-10 text-sm text-slate-500">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between gap-4">
          ...
          <div className="flex gap-6">
            <a href="/login" className="hover:text-ampera-700">Login</a>
            <a href="mailto:info@isipc.com" className="hover:text-ampera-700">Contatti</a>
          </div>
        </div>
      </footer>
    </main>
