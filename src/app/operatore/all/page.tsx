    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="px-5 mb-3 flex gap-2">
        {[
          ...
          <button key={o.v} onClick={() => setFilter(o.v as any)} className={`flex-1 py-2 rounded-lg text-sm font-medium ${filter === o.v ? "bg-ampera-700 text-white" : "bg-white border border-slate-200"}`}>{o.l}</button>
        ))}
      </div>

      <div className="px-5 space-y-3">
        {loading ? <Loader2 className="h-6 w-6 animate-spin mx-auto my-10 text-ampera-700" /> : items.length === 0 ? (
          <div className="text-center py-12 text-slate-500"><Calendar className="h-7 w-7 mx-auto mb-2" />Nessun intervento</div>
        ) : items.map(w => (
          <Link key={w.id} href={`/operatore/work-orders/${w.id}`} className="block bg-white rounded-xl p-4 border border-slate-200">
            <div className="text-xs text-slate-500">{w.scheduledDate ? formatDate(w.scheduledDate) : "Non programmato"}</div>
            <div className="font-semibold">{w.customer.companyName || w.customer.name}</div>
            <div className="text-sm text-slate-600 mt-1 line-clamp-2">{w.title}</div>
