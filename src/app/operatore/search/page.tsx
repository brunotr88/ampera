    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="px-5 space-y-4">
        {results.customers.length > 0 && (
          <section>
            <div className="text-xs font-semibold text-slate-500 uppercase mb-2">Clienti</div>
            {results.customers.map((c: any) => (
              <Link key={c.id} href={`/admin/customers/${c.id}`} className="block bg-white rounded-lg p-3 border mb-2">
                <div className="font-semibold">{c.companyName || `${c.name} ${c.surname || ""}`}</div>
                <div className="text-xs text-slate-500">{c.vatNumber || c.fiscalCode || c.email}</div>
              </Link>
            ))}
          </section>
        )}
        {results.materials.length > 0 && (
          <section>
            <div className="text-xs font-semibold text-slate-500 uppercase mb-2">Articoli</div>
            {results.materials.map((m: any) => (
              <div key={m.id} className="bg-white rounded-lg p-3 border mb-2">
                <div className="font-medium">{m.name}</div>
                <div className="text-xs text-slate-500 font-mono">{m.code} {m.metelCode && `· METEL ${m.metelCode}`}</div>
              </div>
            ))}
          </section>
        )}
